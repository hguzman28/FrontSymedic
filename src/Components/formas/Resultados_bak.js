...
//  END INLCUIR 
 ...
 // const baseurl = 'http://127.0.0.1:8000/';
  ...
  /*
    const handleChangeIncluir=(e)=>{
      //(event.target.checked?'S':'N'):value  
  
      let pos = 0;
      pos = e.target.getAttribute('id');
  
      //respuesta.examenes.map((obj) => {
      //  vectorIncluirExamen[obj.seq] = 'S'; 
      //});
      console.log('vector de inclusion posicion ' + pos + ' estado ' + e.target.checked );
  
      vectorIncluirExamen[pos] = e.target.checked?'S':'N'; 
      setIncluirExamen(vectorIncluirExamen);
      console.log('vector de inclusion');
      console.log(vectorIncluirExamen);
  ...
  return (
    ...
    <> 
      ...
      <div className="table table-hover">
        ...
        <div >
            <button className="btn btn-primary btn-sm" onClick={traedatosenc}>Buscar</button>
            <span>{"  "}</span>
            <button
