...
const analisisexamen = async (secu) => {
      ...
      console.log(' recibio parametros');

      fetch(baseurl+"resultadodetalle/find/" + agencia + "/" + numorden + "/" + seq , {
...
//******************************************
  ...
  function Analisisexam(secu) {

    console.log("Dentro Analisisexam");
    setLoading(true);
                ...
                <td>
                   <label>  
                    <input className="form-control"  type="text" name={analisis.id} id={analisis.id} key={analisis.id} 
                      value={results[analisis.id]} onChange={handleChange}  onKeyUp={regteclares} analisis={analisis.cdganalisis} tipores={analisis.tipores} orden={analisis.orden} />
                   </label>  
